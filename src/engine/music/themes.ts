...
const AMBIENT_LAYERS = { pad: 0, bell: 0.15, bass: 0.22, hat: 0.4, kick: 0.5, arp: 0.5 };
 ...
 */
const BREAKS_VOICES = { pad: 'strings', bell: 'choir', bass: 'drive' } as const;

/** The arpeggio sits well back in both: texture behind the rest, not a part. */
const BREAKS_GAINS = { arp: 0.4 } as const;
const AMBIENT_GAINS = { arp: 0.4 } as const;
...
export const SOUNDTRACKS = {
  ...
  ambient: {
    ...
    moods: {
      menu: {
        ...
        layers: AMBIENT_LAYERS, patterns: AMBIENT_PATTERNS, gains: AMBIENT_GAINS,
      },
      map: {
   ...
   * shooters. Its numbers are not invented: tempo, key, the drum grids and the fact that the
   * *bass* is the hook were all read out of a reference module
   * (`scripts/../assets/music`, parsed with the throwaway IT reader) — 125 BPM, C minor with the
   * sixth and seventh left out, kick four on the floor, and a bass channel carrying twice the
   * notes of anything else.
   *
   * Only those parameters are borrowed. No note of the reference is reproduced: the riffs and
   * chords are generated from the same tables and seed as every other soundtrack's.
   */
  breaks: {
    label: 'Breakbeat',
    ...
    moods: {
      ...
      menu: {
        bpm: 118, root: 48, scale: NATURAL_MINOR, progressions: BREAKS_LOOPS,
        barsPerChord: 4, intensity: 0.26, reverb: 0.4, breath: 0.2, bassMode: 'riff',
        layers: BREAKS_LAYERS, patterns: BREAKS_PATTERNS, voices: BREAKS_VOICES,
        gains: BREAKS_GAINS,
      },
      map: {
        bpm: 125, root: 48, scale: NATURAL_MINOR, progressions: BREAKS_LOOPS,
